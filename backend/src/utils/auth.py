...
class AuthUtils:
    ...
    async def authenticate_user(self, email: str, password: str) -> Mapping[str, Any] | None:
    ...
    def create_access_token(self, claims: dict, expires_delta: timedelta | None = None):
        ...
        to_encode.update({"iat": now.timestamp() ,"exp": expire.timestamp(), "iss": settings.domain_url })
        
        encoded_jwt = jwt.encode(to_encode, key=settings.jwt_secret_key, algorithm=settings.jwt_algorithm )
        return encoded_jwt, timedelta(minutes=15).seconds if not expires_delta else expires_delta.seconds
    


def _auth_util() -> AuthUtils:
    return AuthUtils()